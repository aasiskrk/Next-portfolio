...
export function Introduction({ onViewProjects, onContact }: IntroductionProps) {
  return (
    <section
      id="home"
      className="relative flex min-h-[100dvh] items-center px-5 pb-20 pt-28 sm:px-8"
    >
      <div className="mx-auto grid w-full max-w-6xl items-center gap-14 lg:grid-cols-[1.15fr_0.85fr]">
        ...
        <motion.div variants={container} initial="hidden" animate="visible" className="order-2 lg:order-1">
          ...
          >
            Aashista
            <br />
            <span className="text-outline">Karki</span>
          </motion.h1>

          <motion.h2
            variants={item}
            className="mt-6 text-xl font-medium text-white/80 sm:text-2xl"
          >
          ...
          <motion.p
            variants={item}
            className="mt-5 max-w-xl text-pretty text-base leading-relaxed text-white/50 sm:text-lg"
          >
            I build cross-platform mobile apps with Flutter and full-stack web apps with Next.js
            and the MERN stack. I&apos;ve shipped real products to the Play Store, including{" "}
            <span className="text-white/80">Patro+</span>, a Nepali calendar app with 1.5k+
            downloads.
          </motion.p>
          ...
          <motion.div variants={item} className="mt-9 flex items-center gap-3">
            {socials.map(({ icon: Icon, href, label }) => (
              <a
                ...
                aria-label={label}
                className="glass-social flex h-11 w-11 items-center justify-center rounded-full text-white/70 transition-transform duration-500 ease-fluid hover:-translate-y-0.5 hover:text-white"
              >
        ...
        >
          <div className="glass-card-premium rounded-[2rem] p-2">
            <div className="relative aspect-[4/5] overflow-hidden rounded-[calc(2rem-0.5rem)] glass-inner-premium">
              <Image
                src="/images/me1.jpg"
                alt="Portrait of Aashista Karki"
                fill
                priority
                sizes="(max-width: 1024px) 90vw, 40vw"
                className="object-cover grayscale contrast-[1.05] transition-all duration-700 ease-fluid hover:grayscale-0"
              />
              <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/60 to-transparent" />
              <div className="absolute bottom-4 left-4 font-mono text-[10px] uppercase tracking-[0.3em] text-white/70">
                Est. 2021 · Dev
              </div>
            </div>
          </div>
